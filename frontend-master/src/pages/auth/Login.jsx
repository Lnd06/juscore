...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if (params.get('error') === 'not_authorized') {
      setError('Acesso restrito. Apenas administradores (master) podem acessar esta plataforma.');
    }
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Garante que se o usuário não for master, ele seja barrado
      if (res && res.user && res.user.tipo !== 'master') {
        setError('Acesso restrito. Apenas administradores (master) podem acessar esta plataforma.');
        return;
      }
