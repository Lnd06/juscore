...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    // 1. Initial Auth Check - lemos o cookie compartilhado para cross-origin/cross-port
    const token = getCookie('juscore_token') || localStorage.getItem('token');
    if (token) {
      // Se lemos do cookie, salvamos no localStorage local também por redundância
      localStorage.setItem('token', token);
      axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
      fetchUser();
    } else {
