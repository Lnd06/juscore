...
const AdminDashboard = () => {
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
        ...
        <div className="flex bg-gray-100 dark:bg-gray-700 p-1 rounded-xl">
          <button 
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'overview' ? 'bg-brand-bg dark:bg-juri-800 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-900 dark:hover:text-gray-200'}`}
          >
            Visão Geral
          </button>
          <button 
            onClick={() => setActiveTab('users')}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'users' ? 'bg-brand-bg dark:bg-juri-800 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-900 dark:hover:text-gray-200'}`}
          >
            Usuários
          </button>
          <button 
            onClick={() => setActiveTab('violations')}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'violations' ? 'bg-brand-bg dark:bg-juri-800 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-900 dark:hover:text-gray-200'}`}
          >
            Violações
          </button>
          <button 
            onClick={() => setActiveTab('settings')}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'settings' ? 'bg-brand-bg dark:bg-juri-800 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-900 dark:hover:text-gray-200'}`}
          >
            Configurações
          </button>
          <button 
            onClick={() => setActiveTab('organizations')}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'organizations' ? 'bg-brand-bg dark:bg-juri-800 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-900 dark:hover:text-gray-200'}`}
          >
            Organizações
          </button>
          <button 
            onClick={() => setActiveTab('prices')}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'prices' ? 'bg-accent text-white shadow-sm' : 'text-gray-500 hover:text-gray-900 dark:hover:text-gray-200'}`}
          >
            💰 Preços
          </button>
          <button 
            onClick={() => setActiveTab('announcements')}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'announcements' ? 'bg-accent text-white shadow-sm' : 'text-gray-500 hover:text-gray-900 dark:hover:text-gray-200'}`}
          >
      ...
      {activeTab === 'users' && (
        <Card>
          <div className="p-4 border-b border-brand-border dark:border-brand-border/50 flex flex-col md:flex-row gap-4 justify-between items-center">
             <div className="w-full flex flex-col md:flex-row gap-2">
               <select
                 className="px-4 py-2 border border-brand-border dark:border-brand-border rounded-xl bg-brand-bg dark:bg-brand-bg text-gray-900 dark:text-white focus:ring-2 focus:ring-accent/20 outline-none"
                 value={filterType}
               ...
               <div className="flex-1 flex gap-2">
                 ...
                 />
                 <Button onClick={() => { setPage(1); fetchUsers(1); }}>Buscar</Button>
      ...
      {activeTab === 'settings' && (
        <div className="space-y-6">
          ...
          <Card className="max-w-2xl border-2 border-accent/20 bg-accent/5">
            <div className="p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div className="flex-1">
                <h3 className="font-bold flex items-center gap-2 text-lg text-accent">
                  <Book className="w-5 h-5" />
                  Biblioteca Jurídica (MongoDB)
                </h3>
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  Ativa o serviço de busca interna em arquivos e base de conhecimento. Desative se o banco de dados MongoDB Atlas estiver inacessível.
                </p>
              </div>
              <div className="flex flex-col items-center gap-2">
                <button
                  ...
                  className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors focus:outline-none ${libraryEnabled ? 'bg-green-500' : 'bg-gray-300'}`}
                >
                  <span
                    className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform ${libraryEnabled ? 'translate-x-7' : 'translate-x-1'}`}
                  />
                </button>
                <span className={`text-[10px] font-bold uppercase ${libraryEnabled ? 'text-green-600' : 'text-gray-500'}`}>
                  {loadingLibrary ? 'Processando...' : libraryEnabled ? 'Ativado' : 'Desativado'}
                </span>
              </div>
            </div>
          </Card>
