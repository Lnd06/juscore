...
router.get("/google/callback", async (req, res) => {
  ...
  try {
    ...
    // ✅ SOLUÇÃO DEFINITIVA: redirecionar o popup para o FRONTEND (mesma origem: localhost:5173)
    // Isso permite window.close(), localStorage e postMessage funcionarem corretamente.
    // O problema anterior era que a página de callback estava em localhost:3000 (origem diferente),
    // fazendo window.opener ser null e localStorage ser isolado da janela pai.
    const userParam = encodeURIComponent(JSON.stringify(userData));
    return res.redirect(
      `${frontendUrl}/auth/google/callback?token=${authToken}&user=${userParam}`,
    );
  } catch (error) {
    console.error("Error in Google Callback:", error);
    return res.redirect(
      `${frontendUrl}/auth/google/callback?error=auth_failed`,
    );
  }
});
